...
ggplot() + theme_bw() +
  ...
  labs(fill = "Altitude (m.s.n.m.)", color = "Season") +
...
valid = na.omit(valid)
valid_rf = randomForest(harvest_kg_ha ~ ., data = valid[, -c(1, 3:6)])


# calculate RMSEP and R2

RMSEP_train = RMSEP(train_rf$predict, train$harvest_kg_ha, na.rm = TRUE)
R2_train = R2(train_rf$predict, train$harvest_kg_ha, na.rm = TRUE)

par(mfrow = c(1,1), mar = c(5,5,5,1), cex.axis = 1.4, cex.lab = 1.6)
plot(x = train$harvest_kg_ha, y = train_rf$predict, xlab = "Observed", ylab = "Predicted", main = "Cassava yield - calibration")
abline(0, 1, col = "red")
fit = lm(train_rf$predict ~ train$harvest_kg_ha)
sfit = summary(fit)
# sfit
...
# calculate RMSEP and R2
RMSEP_test = RMSEP(valid_rf$predict, valid$harvest_kg_ha, na.rm = TRUE)
R2_test = R2(valid_rf$predict, valid$harvest_kg_ha, na.rm = TRUE)

par(mfrow = c(1,1), mar = c(5,5,5,1), cex.axis = 1.4, cex.lab = 1.6)
plot(x = valid$harvest_kg_ha, y = valid_rf$predict, xlab = "Observed", ylab = "Predicted", main = "Cassava yield - validation")
abline(0, 1, col = "red")
fit = lm(valid_rf$predict ~ valid$harvest_kg_ha)
sfit = summary(fit)
# sfit
abline(a = fit$coefficients[1], b = fit$coefficients[2], col = "black")
...
shap_values =  kernelshap(train_rf, data[, -c(1, 3:6)])
...
data_with_shap = cbind(data, shap_values_df)
...
sv_dependence(shp, "plant_density_pl_ha", "travel", alpha = 0.5)
ggsave("Graphs//sv_plant_density_pl_ha.png", units = "cm", width = 20, height = 15, dpi = 320)

sv_dependence(shp, "fert_use", "travel", alpha = 0.5)
ggsave("Graphs//sv_fert_use_a.png", units = "cm", width = 20, height = 15, dpi = 320)
...
pldens15000 =  pred_grid_df[which(pred_grid_df$plant_density_pl_ha == 15000), ]
pldens30000 =  pred_grid_df[which(pred_grid_df$plant_density_pl_ha == 30000), ]

pldens_diff = merge(pldens15000, pldens30000, by = c("x", "y",
                                              "elevation", "soc", "sand", "cec", "ph", "acid_exch", "prec_a", "prec_b",
                                              "tmin_a", "tmin_b", "tmax_a", "tmax_b", "tavg_a", "tavg_b", "srad_a", "srad_b",
                                              "pop", "travel",
                                              "season", "field_size_estimate_ha", "slope_position", "slope_steepness",
                                              "soil_depth", "terrace", "fertile_degree", "cycle_duration",  
                                              "previous_cas", "previous_crl", "previous_leg",
                                              "fert_use", "compost_manure", "pesticide", "times_weeded"))


pldens_diff$pldens_diff = pldens_diff$pred_grid.y - pldens_diff$pred_grid.x

pldens_diff_sf = st_as_sf(pldens_diff, coords = c("x", "y"), crs ="+proj=longlat + datum = WGS84 + no_defs")
pldens_diff_sf = as(pldens_diff_sf, "Spatial")
pldens_diff_sf = vect(pldens_diff_sf)
pldens_diff_sf = project(pldens_diff_sf, "EPSG:4326")

ggplot() + theme_bw() +
  ggtitle("Additional yield with a plant density of 30000 vs 15000 pl/ha") +
  geom_spatvector(data = rwa0, fill = "grey90", linewidth = 0.5, color = "black") +
  geom_spatvector(data = pldens_diff_sf, shape = 16, aes(color = pldens_diff), alpha = 0.5, size = 2) +
  geom_spatvector(data = rwa1, fill = NA, linewidth = 0.5, color = "black") +
  scale_color_viridis_c() +
  theme(plot.title = element_text(size = 18, face = "bold"),
        legend.title = element_blank())

ggsave("Maps\\Predictions\\Plant density.png", units = "cm", width = 20, height = 16, dpi = 320)
